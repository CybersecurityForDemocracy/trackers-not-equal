...
def setup_postgres(cursor):
    ...
    '''
    Columns can be used in the following way:
    1. meta_static_collection_status
        - if empty, meta pixel is not installed
        - if true, meta pixel is installed and configured for fdc
        - if false, meta pixel is installed and not configured for fdc
    2. meta_dynamic_collection_status
        - if true, meta pixel is collecting PII form data
    3. google_static_collection_status
        - if empty, google tag is not installed
        - if true or false, google tag is installed (the difference between true and false is not significant, and exists for legacy reasons only)
    4. google_dynamic_collection_status
        - if true, google tag is collecting PII form data
    '''
    cursor.execute("""
